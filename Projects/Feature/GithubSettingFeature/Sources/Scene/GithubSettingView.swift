    ...
    public var body: some View {
        VStack(spacing: 0) {
            VStack(spacing: 8) {
                ...
                MyTextField("", text: $viewModel.githubId)
            }
            .padding(.top, 20)
            Spacer()
            MyButton("완료", type: .CTA, leadingIcon: .check, isEnabled: !viewModel.githubId.isEmpty) {
                await viewModel.completeSetting()
            }
            .padding(.horizontal, 4)
        }
        .padding(.horizontal, 16)
        .myTopBar("Github 설정", background: .backgroundAlt) {
            router.popToStack()
        }
